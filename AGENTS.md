...
Перед любой задачей по проекту сначала taskdb, потом код: скилл `user-tasks-qdrant-local-read` запускай **до** анализа, чтения кода, планирования и любых действий по задаче.
Для обычных задач поиск taskdb/qdrant делегируй субагенту. Для очевидных микроправок допускается прямой краткий MCP-поиск без субагента, но всё равно до чтения/правки кода.
...
По закрытию задачи («+», «готово», «ок», «отлично», «сделано», «=», «закрывай», «задача завершена») — сначала скилл `user-tasks-qdrant-local-write`, затем коммит с кратким сообщением по сути изменений.
