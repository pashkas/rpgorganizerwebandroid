...
export class RequirementsAddDialogComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.data) {
      ...
      const el = elements.find((e) => e.elId == this.data.elId);
      this.ReqForm.get("elId").setValue(el);
    ...
    this.ReqForm.get("type")
      ...
      .subscribe((q) => {
        const elements = this.revSrv.getElements(this.srv.pers$.value, q);
        this.elements$.next(elements);
        this.ReqForm.get("elVal").setValue(this.revSrv.getDefaultValue(q));
        if (elements.length > 0) {
          this.ReqForm.get("elId").setValue(elements[0]);
        }
      });

    this.elements$.pipe(takeUntil(this.unsubscribe$)).subscribe((q) => {
      if (this.ReqForm.get("elId").value == null) {
